...
This repo includes a Railway-ready template with `Dockerfile`, `railway.toml`, and `.env.railway.example`.

```bash
# 1) Create service from this repo in Railway
# 2) Add environment variables from .env.railway.example
# 3) Deploy
```
...
- For quieter initial logs, set `PICOCLAW_HEARTBEAT_ENABLED=false` until you add `HEARTBEAT.md` tasks.
- Gateway mode now exposes HTTP health endpoints on the configured gateway port: `/` and `/healthz`.
