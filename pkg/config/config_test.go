...
func TestLoadConfig_EnvOverridesWithoutFile(t *testing.T) {
	t.Setenv("PICOCLAW_PROVIDERS_OPENROUTER_API_KEY", "test-key")
	t.Setenv("PICOCLAW_AGENTS_DEFAULTS_MODEL", "gpt-5-mini")
	t.Setenv("PICOCLAW_GATEWAY_PORT", "8080")

	nonExistentPath := filepath.Join(t.TempDir(), "missing-config.json")
	if _, err := os.Stat(nonExistentPath); !os.IsNotExist(err) {
		t.Fatalf("expected test config path to not exist")
	}
	...
	if cfg.Providers.OpenRouter.APIKey != "test-key" {
		t.Fatalf("OpenRouter API key = %q, want %q", cfg.Providers.OpenRouter.APIKey, "test-key")
	}
	if cfg.Agents.Defaults.Model != "gpt-5-mini" {
		t.Fatalf("model = %q, want %q", cfg.Agents.Defaults.Model, "gpt-5-mini")
	}
	if cfg.Gateway.Port != 8080 {
		t.Fatalf("gateway port = %d, want %d", cfg.Gateway.Port, 8080)
	}
}
